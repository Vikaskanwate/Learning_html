...
// delete user with age > 23 
app.delete("/users/age/:age",(req,res)=>{
  
    const user = users.filter(u => u.age > parseInt(req.params.age))
    if(user==''){
        return res.send(`no user having age greater than ${req.params.age}`)
    }
    res.status(200).json({
        user:user,
        length:user.length
